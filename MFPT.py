...
class HybridModel():
    def __init__(self, model_2):
        ...
        self.path_tra_His = './Second_dataset'
    ...
    def data_processing(self,path_data,window_step):
        # 文件夹目录
        path = f"{self.path_data}/dataset_2"
        # 得到文件夹下的所有文件名称
    ...
    #ml算法数据格式
    def train_test_data(self,path_data,window_step):
        x_data, y_data, class_dic = self.data_processing(path_data,window_step)
    ...
    # 训练模型
    #ml-model 训练
    def Training_model_1(self,model_1, x_train, y_train, x_val, y_val):
        #SVM
        start_training_model_1 = time.time()
        if model_1 == 'svm':
            model = OneVsRestClassifier(SVC(C=0.1, kernel='rbf', probability=True, random_state=None))
        elif model_1 =='knn':
        ...
        print(f'Training time of {model_1}:%s Seconds'%(duration_training_model_1))
        # save the model to disk
        filename = f'{self.path_model}/{model_1}_.sav'
    ...
    def Training_model_2(self,model_1, model_2, x_train, y_train, num_cla, path_tra_His):   
        if model_2 == 'NN':
            ...
            model.save(f'{self.path_model}/{model_1}_{model_2}_.h5')
            print(f"[INFO] Model has saved in '{self.path_model}/{model_1}_{model_2}_.h5'.")
            duration_model_2 = end_training_model_2-start_training_model_2
            print(f'Training time of model_2:%s Seconds'%(duration_model_2))
            with open(f'{path_tra_His}/{model_1}_{model_2}_.txt', 'wb') as file_pi:
            ...
            end_training_model_2 = time.time()
            duration_training_model_2 = end_training_model_2-start_training_model_2
            print(f'Training time of model_2: {model_2}:%s Seconds'%(duration_training_model_2))
            # save the model to disk
            filename = f'{self.path_model}/{model_1}_{model_2}_.sav'
            pickle.dump(model, open(filename, 'wb'))
        return duration_training_model_2


    def test_hybrid_model_time(self,path_model, model_1, model_2, x_test, y_test, threshold_proba_everyclass):
        filename = f'{path_model}/{model_1}_.sav'
        model1 = pickle.load(open(filename, 'rb'))
        if model_2 =='NN':
            model2 = keras.models.load_model(f'{path_model}/{model_1}_{model_2}_.h5')
        else:
            filename = f'{path_model}/{model_1}_{model_2}_.sav'
            model2 = pickle.load(open(filename, 'rb'))

        start_test_model_1 = time.time()
        proba_everyclass = model1.predict_proba(x_test)
        proba_everyclass_sort = np.sort(proba_everyclass, axis=1)
        end_test_model_1 = time.time()
        dura_test_model_1 = end_test_model_1 - start_test_model_1
        print(f'Inference time of model_1({model_1}) :%s Seconds'%(dura_test_model_1))
        
        start_filter = time.time()
        x_test_pd = pd.DataFrame(x_test)
        x_test_model_2 = pd.DataFrame()
        for i in range(proba_everyclass.shape[0]):
            if proba_everyclass_sort[i][-1]-proba_everyclass_sort[i][-2] < threshold_proba_everyclass:
    ...
    def test_hybrid_model_accuracy(self,path_model, model_1, model_2, x_test, y_test, threshold_proba_everyclass):
        filename = f'{path_model}/{model_1}_.sav'
        model1 = pickle.load(open(filename, 'rb'))
        if model_2 =='NN':
            model2 = keras.models.load_model(f'{path_model}/{model_1}_{model_2}_.h5')
        ...
        print(f"######acc: {accuracy_score(y_test_model_1, pred_test_model_1)}")
        x_test_model_2 = x_test_model_2.values
    ...
    #对比实验：全量数据进入cnn
    def NN_train(self,x_train_val, y_train_val, num_cla, path_model, path_tra_His):
        ...
        history = model.fit(x_train_model_2,y_train_model_2, epochs=100,batch_size=64,validation_data=(x_test_model_2, y_test_model_2),callbacks=[callback])
        print("[INFO] Model training completed !")
        model.save(f'{path_model}/NN_.h5')
        print(f"[INFO] Model has saved in '{path_model}/NN_.h5'.")
        end_nn = time.time()
        dura_nn = end_nn - start_nn
        print(f'Training time of only NN model:%s Seconds'%(dura_nn))
        with open(f'{path_tra_His}/NN_.txt', 'wb') as file_pi:
            pickle.dump(history.history, file_pi)
        return dura_nn

    # #全量数据进cnn验证--重点关注时间和精确度
    def NN_test(self,path_model, x_test, y_test):
        NN_load = keras.models.load_model(f'{path_model}/NN_.h5')
    ...
    def run(self):
        ...
        # 对于每一个load，分割数据集
        x_train_val, x_train, x_val, x_test, y_train_val, y_train, y_val, y_test = self.train_test_data(self.path_data, 700)
        ...
        for model1 in self.model_1:
          ...
          # 训练机器学习模型1
          t1, duration_training_model_1 = self.Training_model_1(model1, x_train, y_train, x_val, y_val)
          ...
          # 开始训练模型2
          duration_training_model_2 = self.Training_model_2(model1, self.model_2, x_train, np.array(y_train), self.num_cla, self.path_tra_His)

          # 评估模型2时间
          if model1 == 'rf':
            threshold_proba_everyclass=self.probability_threshold[0]
          else:
            threshold_proba_everyclass=self.threshold_proba_everyclass
          dura_test_model_1, filter_time, dura_test_model_2, dura_test_model_all = self.test_hybrid_model_time(self.path_model, model1, self.model_2, x_test, y_test, threshold_proba_everyclass)
      
          # 评估混合模型精度
          acc_hybrid_model, acc_model_1, acc_model_2 = self.test_hybrid_model_accuracy(self.path_model, model1, self.model_2, x_test, y_test, threshold_proba_everyclass)
          duration_all.append([f'{model1}+{self.model_2}:',duration_training_model_1,duration_training_model_2,dura_test_model_1, filter_time, dura_test_model_2, dura_test_model_all])
          acc_all.append([f'{model1}+{self.model_2}:',acc_hybrid_model, acc_model_1, acc_model_2])
        # 对于每个load训练全量CNN+评估
        dura_nn = self.NN_train(x_train_val, np.array(y_train_val), self.num_cla, self.path_model, self.path_tra_His)
        dura_NN_test, acc_NN = self.NN_test(self.path_model, x_test, y_test)
        duration_all.append([dura_nn, dura_NN_test])
        acc_all.append(str(acc_NN)) 
        pd.DataFrame(duration_all).to_csv('./duration_d2.csv',index=None)
        pd.DataFrame(acc_all).to_csv('./acc_d2.csv',index=None)
